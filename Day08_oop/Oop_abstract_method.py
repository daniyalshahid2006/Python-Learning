...
class BankAccount(ABC):
    ...
    def display(self):
        print(f"{self.name}'s whose account number is {self.account_number} balance is {self.__balance}")
    def deposit(self,amount):
        if amount > 0:
           self.__balance += amount
        else:
            print(f"please enter a right amount")
...
class Savings(BankAccount):
    ...
    def display(self):
        print(f"{self.name}'s whose account number is {self.account_number} savings are {self.get_balance()}")
    def withdraw(self,amount):
        if amount > 0:
            if self.get_balance() >= amount:
                ...
                print(f" you dont have enough money to withdraw {amount}")
        else:
            print(f"please enter a right amount")
    def interset(self,amount,time):
        for year in range(1,time+1):
            tier = amount // 100000
            rate = tier * 5
            print(f"year {year} : Rs {amount:.2f} with rate {rate}%")
            amount += amount*(rate/100)
class CurrentAccount(BankAccount):
    def __init__(self,name,account_number,balance,limit):
        super().__init__(name, account_number, balance)
        self.limit = limit
    def display(self):
        print(f"{self.name}'s whose account number is {self.account_number} savings are {self.get_balance()}")
    def withdraw(self,amount):
        if amount > 0:
            if self.__balance >= amount:
                self.__balance -= amount
                print(f"{amount} is withdrawn")
                extra = input("do you want to withdraw more money? y/n").lower()
                if extra == "y":
                    amount2 = int(input("how much money to withdraw?"))
                    if amount2 <= self.limit:
                        self.limit -= amount2
                    else:
                        print(f"insufficient money you can withdraw only {self.limit}")
                else:
                    pass
            else:
...
account_number1 = int(input("Enter your account number: "))
balance1 = int(input("Enter your first balance: "))
savings1 = Savings(name1,account_number1,balance1)
savings1.display()
print(savings1.get_balance())
savings1.deposit(1000000)
savings1.display()
savings1.deposit(0)
savings1.deposit(-1222)
savings1.withdraw(100000)
savings1.interset(100000,10)
name2 = input("Enter your second name: ")
account_number2 = int(input("Enter your account number: "))
balance2 = int(input("Enter your second balance: "))
current1 = CurrentAccount(name2,account_number2,balance2)
current1.display()
print(current1.get_balance())
current1.deposit(100000)
current1.display()
current1.deposit(-1222)
current1.display()
current1.withdraw(100000)
current1.display()
